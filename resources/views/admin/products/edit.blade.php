...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-base font-semibold text-forest">Edit Produk</h1>
        <a href="{{ route('admin.products.index') }}" class="text-xs text-emerald-700 hover:underline">← Daftar Produk</a>
    </div>

    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-4 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                ...
                <div>
                    ...
                    <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full text-xs border rounded-md px-3 py-2">{{ old('description', $product->description) }}</textarea>
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="number" name="purchase_price" value="{{ old('purchase_price', $product->purchase_price) }}" step="0.01" min="0" class="w-full text-xs border rounded-md px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="selling_price" value="{{ old('selling_price', $product->selling_price) }}" step="0.01" min="0" class="w-full text-xs border rounded-md px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_price" value="{{ old('discount_price', $product->discount_price) }}" step="0.01" min="0" class="w-full text-xs border rounded-md px-3 py-2" placeholder="Opsional">
                ...
                <div>
                    ...
                    @if($product->image)
                        <div class="flex items-center gap-2 mb-2">
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-20 h-20 object-cover rounded-md">
                            <label class="inline-flex items-center gap-1 text-xs text-red-600">
                                <input type="checkbox" name="remove_image" value="1" class="rounded border-gray-300"> Hapus gambar
                            </label>
                        </div>
                    @endif
            ...
            <div class="space-y-3">
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="number" name="current_stock" value="{{ old('current_stock', $product->current_stock) }}" min="0" class="w-full text-xs border rounded-md px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="min_stock" value="{{ old('min_stock', $product->min_stock) }}" min="0" class="w-full text-xs border rounded-md px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_stock" value="{{ old('max_stock', $product->max_stock) }}" min="0" class="w-full text-xs border rounded-md px-3 py-2" placeholder="Opsional">
                ...
                <div>
                    ...
                    <input type="text" name="pcs" value="{{ old('pcs', $product->pcs) }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                ...
                <div class="border border-gray-200 rounded-lg p-3 space-y-3">
                    <label class="inline-flex items-center gap-2 text-xs font-semibold text-gray-700">
                        <input type="checkbox" name="is_preorder" value="1" class="rounded border-gray-300" @checked(old('is_preorder', $product->is_preorder))>
                        Pre Order
                    </label>
                    <div class="text-[11px] text-gray-500">Terisi: {{ $product->preorder_filled }}{{ $product->preorder_quota ? ' / ' . $product->preorder_quota : '' }}</div>
                    <div data-preorder-fields class="space-y-2">
                        <div>
                            <label class="block text-[11px] text-gray-600 mb-0.5">Estimasi ketersediaan</label>
                            <input type="text" name="preorder_estimate" value="{{ old('preorder_estimate', $product->preorder_estimate) }}" class="w-full text-xs border rounded-md px-3 py-2" placeholder="Contoh: Maret 2025">
                        ...
                        <div>
                            ...
                            <input type="text" name="preorder_open_until" value="{{ old('preorder_open_until', $product->preorder_open_until?->format('Y-m-d')) }}" data-datepicker class="w-full text-xs border rounded-md px-3 py-2">
                        ...
                        <div>
                            ...
                            <input type="number" name="preorder_quota" value="{{ old('preorder_quota', $product->preorder_quota) }}" min="0" class="w-full text-xs border rounded-md px-3 py-2" placeholder="Opsional">
        ...
        <div class="flex gap-2 pt-2">
            <button type="submit" class="px-4 py-2 rounded-lg bg-forest text-cream text-xs font-semibold">Simpan Perubahan</button>
